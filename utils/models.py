...
class Decoder_block(nn.Module):
    ...
    def forward(self, x1, x2):
        x1 = self.upsample(x1)
        # print(x1.shape,x2.shape,self.k,self.s)
...
class Uresnet(nn.Module):
    ...
    def __init__(self, inplanes=5, outplanes=15, layers=[3, 2, 2, 2]):
        super(Uresnet, self).__init__()
        self.inplanes = inplanes
        self.outplanes = outplanes
        self.encoder1 = self._make_encoder(Block, 32, layers[0], 5)
        self.encoder2 = self._make_encoder(Block, 64, layers[1], 4)
        self.encoder3 = self._make_encoder(Block, 128, layers[2], 4)
        self.encoder4 = self._make_encoder(Block, 256, layers[3], 4)
        self.decoder3 = Decoder_block(256, 128, stride=3, kernel_size=4)
        self.decoder2 = Decoder_block(128, 64, stride=4, kernel_size=4)
        self.decoder1 = Decoder_block(64, 32, stride=4, kernel_size=4)
        self.conv1x1 = nn.ConvTranspose1d(32, self.outplanes, kernel_size=4, stride=5, bias=False)
...
def swin_transformer_1d():
    ...
    res = Uresnet(inplanes=1,outplanes=1)
    x = torch.rand(2, 1, 728)
    print(net(x).shape)
    print(res(x).shape)
