...
class SFTLoss(torch.nn.Module):
    # calculate the similarity between the feature map and the dictionary after short time fourier transform
    def __init__(self, SFTLoss_weight=1):
        super(SFTLoss, self).__init__()
        self.SFTLoss_weight = SFTLoss_weight
        self.MSE = torch.nn.MSELoss()
    def forward(self, x, y):
        return self.calc_sft(x, y)

    def calc_sft(self, x, y):
        sft_loss = 0
        if len(x.shape) > 2:
            for i in range(x.shape[1]):
                x_i = x[:, i, :]
                y_i = y[:, i, :]
                sft_loss += self.calc_sft(x_i, y_i)
            return sft_loss
        else:
            sft_x = torch.stft(x, n_fft=256, center=False)
            sft_y = torch.stft(y, n_fft=256, center=False)
            return self.SFTLoss_weight * self.MSE(sft_x, sft_y)
...
def calc_loss(pred, target, metrics, bce_weight=0.2):
    ...
    loss = 10*mse(pred, target) + 0.1*sftloss(pred, target)
    return loss
