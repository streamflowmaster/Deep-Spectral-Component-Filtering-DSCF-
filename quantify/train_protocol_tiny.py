...
def run_protocol(
    ...
    model = Hierarchical_1d_model(inplanes=1,outplanes=6,
                          encoder_name=encoder_name,
                          decoder_name=decoder_name,layers=layers,mask=0)

    model_pth = '[Enc_Dec:'+encoder_name+'_'+decoder_name+'][SNR:'+str(snr)+']'+\
                scale+'.pt'
...
if __name__ == '__main__':
    ...
    scales = ['tiny','huge','large',]
    # snrs = [10,15,20,25,30,35]
    snr = 25
    for encoder_name in encoder_names:
        for decoder_name in decoder_names:
            for scale in scales:
                save_head_path = '../DeArtifact_SNR' + str(snr)
