import torch
from ComponentFiltering.BackgroundRemoval_SERS_NP.DSCF_models_pe_ import Hierarchical_1d_model
import os
from ComponentFiltering.BackgroundRemoval_SERS_NP.forward import forward

def run_protocol(
        encoder_name,decoder_name,
        save_head_path='../SERS_bg_removal/',
        batch_size=256,dict_size=15,
        epoch = 2000, device = 'cuda:0',lr = 3e-4,snr = 25,scale = 'large',
    ...
    model = Hierarchical_1d_model(inplanes=1,outplanes=1,
                                  ...
                                  mask=0,patch_size=patch_size,embed_dim=embed_dim)
    ...
    if os.path.exists(model_save_path):
        ...
        forward(batch_size=16,model=model,device=device
              )
...
def fetch_model(
    ...
    model = Hierarchical_1d_model(inplanes=1,outplanes=1,
                                  ...
                                  decoder_name=decoder_name,
                                  layers=layers,d_layers=[2,3,6,2],
                                  mask=0,patch_size=patch_size,embed_dim=embed_dim,sig_len=1799)
