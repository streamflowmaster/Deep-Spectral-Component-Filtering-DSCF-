...
from ComponentFiltering.pretrain.DSCF_models_pe_ import DSCFConfig, MultiDec_1d_model
...
class Hierarchical_1d_model(nn.Module):
    ...
    def  __init__(self,inplanes = 1, outplanes = 1,encoder_name = 'SiT',embed_dim = 128,
                 decoder_name = 'TConv',layers=[3,2,2,2],d_layers=[2,2,2],sig_len = 709,device ='cuda:0',
    ...
    def load_encoder_from_pretrain(self,pretrain_path):
        pretrain = torch.load(pretrain_path)['model']
        print(pretrain.keys())

        MultiDec_1d_model.load_state_dict(pretrain)
        self.enc1.load_state_dict(pretrain['enc1'])
        self.enc2.load_state_dict(pretrain['enc2'])
        self.enc3.load_state_dict(pretrain['enc3'])
        self.enc4.load_state_dict(pretrain['enc4'])
...
if __name__ == '__main__':
    ...
    net = Hierarchical_1d_model(inplanes=8,encoder_name='SiT',decoder_name='PPS',device='cpu',patch_size=4,embed_dim=16)
    ...
    net = Hierarchical_1d_model(inplanes=8,encoder_name='SiT',decoder_name='PPS',device='cpu',patch_size=4,embed_dim=256)
    print(net(x).shape)

    net = Hierarchical_1d_model(inplanes=8,encoder_name='SiT',decoder_name='PPS',device='cpu',patch_size=4,embed_dim=128)
    print(net(x).shape)