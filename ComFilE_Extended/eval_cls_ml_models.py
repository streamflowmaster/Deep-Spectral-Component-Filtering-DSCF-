...
def test_one_epoch(model, device, train_data,f_model,component_idx,visual = False):
    ...
    for idx,(inputs,target) in enumerate(train_data):
        ...
        if component_idx is not None:
            # judge if the component_idx is a single number
            if type(component_idx) == int:
                component_idx = [component_idx]
            elif type(component_idx) == torch.Tensor:
                component_idx = component_idx.cpu().numpy().tolist()
            # remove redundant components
            component_idx = list(set(component_idx))
            filtered_inputs = inputs
            # print(component.shape)

            B, _, Li = inputs.shape

            for c_idx in component_idx:
                componnets = f_model(inputs)
                if type(component_idx) == torch.Tensor:
                    component = componnets[:,c_idx].sum(1).unsqueeze(1)
                else:
                    component = componnets[:,c_idx].unsqueeze(1)
                B, C, Lc = componnets.shape
                if Lc != Li: component = length_adopt(B,Lc,Li,component,device).to(device)
                filtered_inputs = filtered_inputs - component


                if visual and c_idx ==0:
