...
class ConvTranspose1dDecoder(nn.Module):
    def __init__(self, inplanes, outplanes,act = nn.GELU,depth=2):
        ...
        self.modules = nn.ModuleList([
            conv1d_5_keep_size(inplanes, outplanes)
            for i in range(depth)])
...
class SiT1dDecoder(nn.Module):
    def __init__(self, inplanes, outplanes,act = nn.GELU,depth = 2):
        ...
        self. blks = nn.ModuleList([
            SwinTransformerBlock1D(num_heads=16,shift_size=1,dim=outplanes)
            for i in range(depth)])
        self.act = act()
...
class PatchEmbed1D(nn.Module):
    ...
    def __init__(self,
                 patch_size=4, in_chans=32, embed_dim=128, norm_layer=None,device = 'cuda:0'
                 ):
        super().__init__()
...
class Hierarchical_1d_model(nn.Module):
    ...
    def  __init__(self,inplanes = 1, outplanes = 1,encoder_name = 'SiT',embed_dim = 128,
                 decoder_name = 'TConv',layers=[3,2,2,2],d_layers=[1,1,1,1],sig_len = 709,device ='cuda:0',
        ...
        if self.encoder_name == 'SiT':
            ...
            self.dec0 = self.conv1x1 = nn.ConvTranspose1d(embed_dim*2, out_channels=outplanes, kernel_size=1, stride=1,
                                                          bias=False).to(device)
...
class MultiDec_1d_model(nn.Module):

    def  __init__(self,config):

        super(MultiDec_1d_model, self).__init__()
        inplanes = config.inplanes
        outplanes = config.outplanes
        ...
        self.device = device
        if self.encoder_name == 'SiT':
            self.patch_embed = PatchEmbed1D(in_chans=inplanes,patch_size=patch_size,embed_dim=embed_dim,device=device)
            self.unpatchy = UnPatchEmbed1D(out_chans=outplanes, embed_dim=embed_dim, patch_size=patch_size,
                                           device=device)
        ...
        self.Dec_ir = nn.Sequential(
            self._make_decoder(block_name=decoder_name, dim=embed_dim * 32, blocks=d_layers[0]).to(device),
            self._make_decoder(block_name=decoder_name, dim=embed_dim * 16, blocks=d_layers[1]).to(device),
            self._make_decoder(block_name=decoder_name, dim=embed_dim * 8, blocks=d_layers[2]).to(device),
            self._make_upsample(block_name=decoder_name, dim=embed_dim * 4).to(device))

        self.Dec_sers = nn.Sequential(
            self._make_decoder(block_name=decoder_name, dim=embed_dim * 32, blocks=d_layers[0]).to(device),
            self._make_decoder(block_name=decoder_name, dim=embed_dim * 16, blocks=d_layers[1]).to(device),
            self._make_decoder(block_name=decoder_name, dim=embed_dim * 8, blocks=d_layers[2]).to(device),
            self._make_upsample(block_name=decoder_name, dim=embed_dim * 4).to(device))

        self.Dec_raman = nn.Sequential(
            self._make_decoder(block_name=decoder_name, dim=embed_dim * 32, blocks=d_layers[0]).to(device),
            self._make_decoder(block_name=decoder_name, dim=embed_dim * 16, blocks=d_layers[1]).to(device),
            self._make_decoder(block_name=decoder_name, dim=embed_dim * 8, blocks=d_layers[2]).to(device),
            self._make_upsample(block_name=decoder_name, dim=embed_dim * 4).to(device))

        self.Dec_uv = nn.Sequential(
            self._make_decoder(block_name=decoder_name, dim=embed_dim * 32, blocks=d_layers[0]).to(device),
            self._make_decoder(block_name=decoder_name, dim=embed_dim * 16, blocks=d_layers[1]).to(device),
            self._make_decoder(block_name=decoder_name, dim=embed_dim * 8, blocks=d_layers[2]).to(device),
            self._make_upsample(block_name=decoder_name, dim=embed_dim * 4).to(device))
    ...
    def linear_upsample(self,x):
        x = torch.nn.functional.interpolate(x,size=(self.sig_len),mode='linear')
        return x
    ...
    def decode(self,down,types=None):
        ...
        idx_ir = np.argwhere(types == 'ir')
        idx_raman = np.argwhere(types == 'raman')
        idx_sers = np.argwhere(types == 'sers')
        idx_uv = np.argwhere(types == 'uv')

        if len(idx_ir) > 0:
            up3[idx_ir] = self.Dec_ir[0](down4[idx_ir], down3[idx_ir])
            up2[idx_ir] = self.Dec_ir[1](up3[idx_ir], down2[idx_ir])
            up1[idx_ir] = self.Dec_ir[2](up2[idx_ir], down1[idx_ir])
            if self.decoder_name in ['PatchPixelShuffle', 'PPS']:
                up1[idx_ir] = up1[idx_ir].permute(0, 2, 1)
            up0[idx_ir] = self.Dec_ir[3](up1[idx_ir])

        if len(idx_raman) > 0:
            up3[idx_raman] = self.Dec_raman[0](down4[idx_raman], down3[idx_raman])
            up2[idx_raman] = self.Dec_raman[1](up3[idx_raman], down2[idx_raman])
            up1[idx_raman] = self.Dec_raman[2](up2[idx_raman], down1[idx_raman])
            if self.decoder_name in ['PatchPixelShuffle', 'PPS']:
                up1[idx_raman] = up1[idx_raman].permute(0, 2, 1)
            up0[idx_raman] = self.Dec_raman[3](up1[idx_raman])

        if len(idx_sers) > 0:
            up3[idx_sers] = self.Dec_sers[0](down4[idx_sers], down3[idx_sers])
            up2[idx_sers] = self.Dec_sers[1](up3[idx_sers], down2[idx_sers])
            up1[idx_sers] = self.Dec_sers[2](up2[idx_sers], down1[idx_sers])
            if self.decoder_name in ['PatchPixelShuffle', 'PPS']:
                up1[idx_sers] = up1[idx_sers].permute(0, 2, 1)
            up0[idx_sers] = self.Dec_sers[3](up1[idx_sers])

        if len(idx_uv) > 0:
            up3[idx_uv] = self.Dec_uv[0](down4[idx_uv], down3[idx_uv])
            up2[idx_uv] = self.Dec_uv[1](up3[idx_uv], down2[idx_uv])
            up1[idx_uv] = self.Dec_uv[2](up2[idx_uv], down1[idx_uv])
            if self.decoder_name in ['PatchPixelShuffle', 'PPS']:
                up1[idx_uv] = up1[idx_uv].permute(0, 2, 1)
            up0[idx_uv] = self.Dec_uv[3](up1[idx_uv])
    ...
    def load_pretrained_encoder(self,state_dict):
        encs = [self.enc1,self.enc2,self.enc3,self.enc4]
        prefix_keys = ['enc.0.','enc.1.','enc.2.','enc.3.']
        encs_dict = []
        for idx,prefix in enumerate(prefix_keys):
            to_load_dict = {}
            state_keys = list(encs[idx].state_dict().keys())
            for state in state_keys:
                # print(prefix+state)
                if prefix+state in state_dict.keys():
...
if __name__ == '__main__':
    ...
    net = MultiDec_1d_model(inplanes=8, encoder_name='SiT', decoder_name='PPS', device='cpu')
    ...
    # print(net)
    print(net(x,types=['ir','raman','uv','ir']).shape)
